...
def get_training_input(filenames, params):
    ...
    with tf.device("/cpu:0"):
        ...
        # Append <eos> symbol
        dataset = dataset.map(
            lambda *x: [tf.concat([y, [tf.constant(params.eos)]], axis=0) for y in x],
            num_parallel_calls=params.num_threads
        )
        ...
        for i in range(len(filenames)-2):
            ...
            features["mt_length_%d" % i] = tf.to_int32(features["mt_length_%d" % i])
            features["mt_length_%d" % i] = tf.squeeze(features["mt_length_%d" % i], 1)
...
def get_evaluation_input(inputs, params):
    with tf.device("/cpu:0"):
        ...
        def convert_to_dict(src, *x):
            res = {}
            res["source"] = src
            res["source_length"] = tf.shape(src)[0]
            for i, v in enumerate(x[:n]):
                res["mt_%d" % i] = v
                res["mt_length_%d" % i] = tf.shape(v)[0]
            res["references"] = x[n:]
            return res
        ...
        pad_dim_dict = {"source": [tf.Dimension(None)], "source_length": [], "references": (tf.Dimension(None),) * (len(inputs) - 1 - n)}
        for i in range(n):
            pad_dim_dict["mt_%d" % i] = [tf.Dimension(None)]
            pad_dim_dict["mt_length_%d" % i] = []
        pad_val_dict = {"source": params.pad, "source_length": 0, "references": (params.pad,) * (len(inputs) - 1 - n)}
        for i in range(n):
            pad_val_dict["mt_%d" % i] = params.pad
            pad_val_dict["mt_length_%d" % i] = 0
...
def get_inference_input(inputs, params):
    ...
    with tf.device("/cpu:0"):
        ...
        n = params.sc_num
        pad_dim_dict = {"source": [tf.Dimension(None)], "source_length": []}
        for i in range(n):
            pad_dim_dict["mt_%d" % i] = [tf.Dimension(None)]
            pad_dim_dict["mt_length_%d" % i] = []
        pad_val_dict = {"source": params.pad, "source_length": 0}
        for i in range(n):
            pad_val_dict["mt_%d" % i] = params.pad
            pad_val_dict["mt_length_%d" % i] = 0
