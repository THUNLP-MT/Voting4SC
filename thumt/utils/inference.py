...
def _get_inference_fn(model_fns, features):
    def inference_fn(inputs, state):
        ...
        for i in range(100):
            if features.has_key("mt_%d" % i):
                local_features["mt_%d" % i] = features["mt_%d" % i]
                local_features["mt_length_%d" % i] = features["mt_length_%d" % i]
            else:
                break
...
def create_inference_graph(models, features, params):
    ...
    for i in range(n):
        features["mt_%d" % i] = tf.expand_dims(features["mt_%d" % i], 1)
        features["mt_%d" % i] = tf.tile(features["mt_%d" % i], [1, beam_size, 1])
    ...
    for i in range(n):
        shape = tf.shape(features["mt_%d" % i])
        features["mt_%d" % i] = tf.reshape(features["mt_%d" % i],
                                    [shape[0] * shape[1], shape[2]])
    ...
    features["source_length"] = tf.tile(features["source_length"],
                                        [1, beam_size])
    for i in range(n):
        features["mt_length_%d" % i] = tf.expand_dims(features["mt_length_%d" % i], 1)
        features["mt_length_%d" % i] = tf.tile(features["mt_length_%d" % i],
                                        [1, beam_size])
    ...
    features["source_length"] = tf.reshape(features["source_length"],
                                           [shape[0] * shape[1]])
    for i in range(n):
        shape = tf.shape(features["mt_length_%d" % i])
        features["mt_length_%d" % i] = tf.reshape(features["mt_length_%d" % i],
                                           [shape[0] * shape[1]])
