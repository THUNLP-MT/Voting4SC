...
def multiplicative_attention(queries, keys, values, bias, keep_prob=None,
                             name=None, rpr=None, similarity=None):
    ...
    with tf.name_scope(name, default_name="multiplicative_attention",
        ...
        if similarity is not None:
            _logits = tf.reshape(logits, [bs, hd*lq, lk]) # bs, hd*lq, lk
            # similarity - (bs, lk, lk)
            _additional_sim_logits = tf.matmul(_logits, similarity, transpose_b=True)
            additional_sim_logits = tf.reshape(_additional_sim_logits, [bs, hd, lq, lk])
            logits += additional_sim_logits
...
def multihead_attention(queries, memories, bias, num_heads, key_size,
                        ...
                        max_relative_dis=None, similarity=None):
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        if max_relative_dis and memories is None:
            ...
            # attention
            results = multiplicative_attention(q, k, v, bias, keep_prob, similarity=similarity)
